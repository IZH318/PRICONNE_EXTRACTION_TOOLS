...
- **`.\src\dataminer.py`**
  - **병렬 처리 제거 및 순차 처리로 변경**:
    - `multiprocessing.Pool`을 제거하고, 병렬 처리 대신 순차적으로 처리하는 방식으로 변경.
    - `imap`을 사용한 비동기 처리 대신, `for` 루프를 통해 순차적으로 `assetbundle` 및 `files` 처리.
    - 코드 가독성 향상과 단순화를 위한 변경.
